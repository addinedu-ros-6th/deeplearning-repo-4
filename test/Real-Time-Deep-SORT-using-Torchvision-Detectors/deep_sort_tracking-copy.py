...
from torchreid.utils import FeatureExtractor  # FeatureExtractor를 사용
...
# Initialize the FeatureExtractor for OSNet-AIN
if args.embedder == 'osnet_ain':
    use_gpu = device.type == 'cuda'  # GPU 사용 여부를 확인
    embedder = FeatureExtractor(
        model_name='osnet_ain_x1_0',
        device=device,
        use_gpu=use_gpu  # 여기서 use_gpu는 True 또는 False
    )
else:
    embedder = args.embedder
# Define a function to extract embeddings using the FeatureExtractor
def extract_embeddings(input_tensor):
    embeddings = embedder(input_tensor)
    return embeddings

# Initialize the Deep SORT tracker with the FeatureExtractor
tracker = DeepSort(max_age=50, embedder=extract_embeddings)
