...
GUI_SERVER_PORT = 8889  # 헬스 체크용 TCP 포트
FRAME_WIDTH = 640  # 수신되는 영상의 너비
FRAME_HEIGHT = 480  # 수신되는 영상의 높이


class MainWindow(QMainWindow):
    def __init__(self, gui_req_queue, gui_send_queue):
        super().__init__()
        self.gui_send_queue = gui_send_queue
        self.gui_req_queue = gui_req_queue
    ...
    def show_input_face(self):
        self.input_face_dialog = InputFaceDialog(self.gui_send_queue)
    ...
    def show_find_man(self):
        ...
        self.find_man_window = FindManWindow(self.gui_req_queue)
...
class InputFaceDialog(QDialog):
    def __init__(self, gui_send_queue, parent=None):
...
class FindManWindow(QMainWindow):
    def __init__(self, gui_req_queue):
        super().__init__()
        self.gui_req_queue = gui_req_queue
        ...
        self.timer = QTimer(self)
        self.timer.timeout.connect(self.update_frame)
        self.timer.start(10)  # Update every 30 milliseconds

    # 프레임을 QLabel에 업데이트하는 함수
    def update_frame(self):
        
        if self.gui_req_queue.size() > 0:
            frame = self.gui_req_queue.get()
...
def gui_client(gui_req_queue, gui_send_queue):
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client_socket.connect((SERVER_IP, GUI_SERVER_PORT))
    print("GUI client 서버에 연결됨")
        ...
        while True:
            ...
            try:
                ...
                while True:
                    ...
                    buffer = buffer[idx:]  # 처리된 JSON 데이터를 버퍼에서 제거
                    ...
                    # 우선 frame 만 던짐 
                    gui_req_queue.put(frame)
                    print(f"gui_req_queue size : {gui_req_queue.size()}")
            except json.JSONDecodeError:
                # 아직 완전한 JSON 데이터가 수신되지 않았음, 더 받아야 함
                continue

    except Exception as e:
        print(f"GUI 클라이언트 수신 오류: {e}")
...
if __name__ == "__main__":
    gui_send_queue = CircularQueue(max_size=10)
    gui_req_queue = CircularQueue(max_size=10)

    threading.Thread(target=gui_client, args=(gui_req_queue, gui_send_queue,)).start() 

    app = QApplication(sys.argv)
    window = MainWindow(gui_req_queue, gui_send_queue)
    window.show()
    sys.exit(app.exec_())