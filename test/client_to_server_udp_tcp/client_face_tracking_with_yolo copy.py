...
def process_frame(frame):
    """
    YOLOv8를 사용하여 사람을 감지하고, OSNet을 통해 ID를 부여하는 함수.
    """
    ...
    results = model.track(frame, persist=True, conf=0.3)  # classes=[0] 옵션 제거
    ...
    for result in results:
        for track in result.boxes:
            ...
            # 바운딩 박스 좌표 계산
            x_min, y_min, x_max, y_max = track.xyxy[0].cpu().numpy()
            x_mid = int((x_min + x_max) / 2)
            y_mid = int((y_min + y_max) / 2)

            # 바운딩 박스 내의 사람 이미지 추출
            person_image = frame[int(y_min):int(y_max), int(x_min):int(x_max)]
            ...
            cv2.putText(frame, f"ReID: {matched_id}", (int(x_min), int(y_min) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
