...
class ClothPopDialog(QDialog):
    ...
    def get_color_from_image1(self, event):
        # QLabel에서 클릭한 좌표 가져오기
        x = event.pos().x()
        y = event.pos().y()

        # 이미지에서 해당 좌표의 픽셀 색상 가져오기
        pixmap = self.spectrum_image1.pixmap()
        if pixmap:
            image = pixmap.toImage()
            color = image.pixelColor(x, y)

            # QFrame의 배경색 설정
            self.color_frame1.setStyleSheet(f"background-color: {color.name()};")
            print(f"Color1: {color.name()}")
    ...
    def get_color_from_image2(self, event):
        ...
        # 이미지에서 해당 좌표의 픽셀 색상 가져오기
        pixmap = self.spectrum_image2.pixmap()
        if pixmap:
            image = pixmap.toImage()
            color = image.pixelColor(x, y)

            # QFrame의 배경색 설정
            self.color_frame2.setStyleSheet(f"background-color: {color.name()};")
            print(f"Color2: {color.name()}")
